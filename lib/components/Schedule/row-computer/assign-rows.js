...
var sortBy = function sortBy(prop) {
  return _ramda2.default.sortWith(prop === 'timestamp' ? [_ramda2.default.ascend(timestamp), _ramda2.default.ascend(_ramda2.default.prop('id'))] : [_ramda2.default.ascend(_ramda2.default.prop(prop))]);
};
...
var assignRow = function assignRow(rows, _ref) {
  ...
  return function (acc, event) {
    if (moveToTopId && moveToTopId === event.id) {
      return _ramda2.default.assoc(event.id, _extends({}, event, { row: 0 }), acc);
    }
    if (isolateId && isolateId === event.id) {
      return _ramda2.default.assoc(event.id, _extends({}, event, { row: isolateRow }), acc);
    }
    ...
    return _ramda2.default.assoc(event.id, _extends({}, event, { row: i }), acc);
  };
};
