...
class Events extends Component {
  ...
  componentWillReceiveProps = nextProps => {
    if (typeof nextProps.scrollLeft === 'number') {
      this.setState({ phantomPosition: nextProps.scrollLeft + 1500 }, () => {
    ...
    return ({ index }) => {
      ...
      if (!datum)
        return {
          ...
          y: Math.max(550, this.maxRow * 75 + 50)
        }
  ...
  handleStop = index => extend => (_, drag) => {
    ...
    const update = this.props.onUpdate(updatedEvent)
  ...
  render() {
    const events = this.props.events
    const cellCount = values(events).length + this.state.extraCells + 2
