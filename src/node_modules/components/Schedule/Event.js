...
class Event extends Component {
  ...
  handleStart = extendDir => () => {
    this.props.handleStart(this.props.event.id)
  ...
  render() {
    const { event, style, handleStop, translateY, scrollLeft } = this.props
    const left = this.getLeft(style.left)
    const width = this.getWidth(style.width)
    const eventOffset = Math.max(0, scrollLeft - style.left)
    const textOffset = eventOffset + 50 > width ? 0 : eventOffset
    ...
    const fromTodayPixelCount =
      ...
      getPixelCount(
        moment()
          .startOf('day')
          .isSameOrBefore(event.rangeStart)
          ? moment().startOf('day')
    ...
    const moveToTop = () => this.props.moveToTop(event.id)
    return (
      <div
